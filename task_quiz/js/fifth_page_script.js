function load_script_for_user(){  

    let dict_answer_for_user = {
    ...
    let list_true_question_for_user = ['Москва', 'Красный', 'Банан', 'Жираф', 'Солнце', 'Январь', 'Среда', 'Книга', 'Собака', 'Футбол']

    let list_question_for_user = [
    ...
    var count_question = list_question_for_user.length  
    ...
    document.querySelector('.start-window-test__start-test-btn').addEventListener('click',load_test_for_user)
    document.querySelector('.main-block-question__accept-user-btn').addEventListener('click',check_user_answer)   
    document.querySelector('.result-test-block__btn-block').addEventListener('click',load_start_window)

}
document.addEventListener("DOMContentLoaded",load_script_for_user)
